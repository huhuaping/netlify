...
后期可能还会面临hugo academic升级的问题。相关升级问题记录如下：

1.更新hugo version。在windows 命令行里进行hugo升级（见[官方说明](https://wowchemy.com/docs/update/)）。
...
2.更新hugo academic模板。如前所述，基本过程就是github下载官方模板。然后与你的现有repo进行比对和替换。

3.更新blogdown包。照常使用Rstudio addins `serv site`。需要注意的是新的hugo流程默认是在缓存里渲染网站，也就意味着`public/`文件夹默认不会随着渲染而自动更新。那么需要执行的是**本地渲染**（参看 [网络问答](https://stackoverflow.com/questions/64500959/problem-with-rendering-hugo-blogdown-site)）。
