...
1.使用RStudio插件(addins)中blogdown下的`New post`暂时还会带来困扰（参看["stackoverflow队长"问答](https://stackoverflow.com/questions/64688201/blogdown-new-post-addin-creates-but-not-loads-new-file)）。按照hugo的新流程，每个blog都会建立自己的对应的文件夹（参看[网页](https://ropensci.org/technotes/2020/04/23/rmd-learnings/)）。具体文件夹结构如下：
...
2.使用`knitr::include_graphics`函数调用图片，需要额外设定参数`error = FALSE`。（参看[网络问答](https://stackoverflow.com/questions/60292022/knitr-cannot-find-img-files-in-static-folder)）
