...
使用RStudio插件(addins)中blogdown下的`New post`暂时还会带来困扰（参看["stackoverflow队长"问答](https://stackoverflow.com/questions/64688201/blogdown-new-post-addin-creates-but-not-loads-new-file)）。按照hugo的新流程，每个blog都会建立自己的对应的文件夹（参看[网页](https://ropensci.org/technotes/2020/04/23/rmd-learnings/)）。具体文件夹结构如下：
...
- 新旧文件夹体系的不一致性（见上面树形结构图）。虽然只是视觉上的差异，但是还是不够“一致”！
...
因此，对于分节较多的长篇博文，自然就有了"侧边浮动目录"（sidebar floating toc）的个性化页面样式修订念头。yihui[建议](https://github.com/rstudio/blogdown/issues/375#issuecomment-476332000)可以通过JavaScript或CSS进行设定。Xiaoou WANG也提供了一个hugo语法下的[实现方法](https://xiaoouwang.github.io/post/add_a_float_toc_to_academic_theme/)，但是有不少bug。
